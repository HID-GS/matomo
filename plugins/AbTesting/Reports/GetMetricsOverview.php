...
class GetMetricsOverview extends Base
{
    ...
    protected function init()
    {
        ...
        $this->order = 1;
        $this->access = StaticContainer::get(AccessValidator::class);

        // If this is for the evolution graph, make sure to add the idExperiment parameter
    ...
    {
        $idSite = Common::getRequestVar('idSite', 0, 'int');
        if (empty($idSite) || $this->access->isAbTestingDisabled($idSite)) {
            return;
        }
            ...
            $widgetsList->addWidgetConfig(
                $factory->createWidget()
                    ...
                    ->setParameters(array(
                        ...
                        'columns' => self::DEFAULT_WIDGET_METRICS
                    ))
                    ->setOrder(5)
            );
    ...
    public function configureView(ViewDataTable $view)
    {
        ...
        $idExperiment = Common::getRequestVar('idExperiment', 0, 'int');
        ...
        $request = \Piwik\Request::fromRequest();
        $keetTotalsRow = $request->getIntegerParameter('keep_totals_row', -1);

        if ($keetTotalsRow === -1) {
            $view->requestConfig->request_parameters_to_modify['keep_totals_row'] = 1;
            $view->config->custom_parameters['keep_totals_row'] = 1;
        }
        ...
        $view->requestConfig->request_parameters_to_modify['idExperiment'] = Common::getRequestVar('idExperiment', 0, 'int');
        $view->requestConfig->filter_sort_column = 'label';
        $view->requestConfig->filter_sort_order = 'asc';
        $view->requestConfig->filter_limit = count(!empty($experiment['variations']) ? $experiment['variations'] : []) + 2;
