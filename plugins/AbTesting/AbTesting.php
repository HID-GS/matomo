...
class AbTesting extends Plugin
{
    ...
    public const TRACKER_READY_HOOK_METHOD_NAME = '/*!! abTestingTrackerReadyHookMethod */';
    public const TRACKER_READY_HOOK_METHOD_NAME_WHEN_MINIFIED = '/*!!! abTestingTrackerReadyHookMethod */';
    ...
    public function registerEvents()
    {
        return array(
            ...
            'CompliancePolicy.setActiveStatus' => 'clearAbTestingForSite'
        );
    }

    private static function endAbTestingForSite($idSite, bool $skipSettingCheckForActivePolicy = false)
    {
        if (Site::getTypeFor($idSite) === 'proxysite') {
            return; //do not delete ab tests for proxySite as it will throw an exception due to read only behaviour
        }

        if ($skipSettingCheckForActivePolicy) {
            /*
             * do permissions check, but skip checkIsAbTestingEnabled(), as it
             * will always throw exception when policy is enabled.
             */
            /** @var AccessValidator */
            $accessValidator = StaticContainer::get(AccessValidator::class);
            try {
                $accessValidator->checkSiteExists($idSite);
            ...
            /*
             * Since this block is only used when a core policy is enabled, check
             * the value of the policy setting and only proceed if the setting
             * returns true (policy is enabled or system setting is enabled)
             */
            $disableAbTestingValue = DisableAbTesting::getInstance($idSite)->getValue();
            if (!$disableAbTestingValue) {
                return;
            }

            /** @var Experiments $experimentsModel */
            $experimentsModel = StaticContainer::get('Piwik\Plugins\AbTesting\Model\Experiments');
            /** @var \Piwik\Archive\ArchiveInvalidator $archiveInvalidator */
            $archiveInvalidator = StaticContainer::get('Piwik\Archive\ArchiveInvalidator');
            /** @var Configuration $configuration */
            $configuration = StaticContainer::get('Piwik\Plugins\AbTesting\Configuration');

            $experiments = $experimentsModel->getAllExperimentsForSite($idSite);
            foreach ($experiments as $experiment) {
                $idExperiment = (int) $experiment['idexperiment'];
                $experimentsModel->deleteExperiment($idExperiment, $idSite);
                $archiveInvalidator->removeInvalidationsSafely([$idSite], 'AbTesting', Archiver::getExperimentRecordName($idExperiment));
                if ($configuration->isEstimatedUniqueVisitorArchivingEnabled()) {
                    $archiveInvalidator->removeInvalidationsSafely([$idSite], 'AbTesting', Archiver::getExperimentEstimatedUniqueVisitorBucketRecordName($idExperiment));
                    $archiveInvalidator->removeInvalidationsSafely([$idSite], 'AbTesting', Archiver::getExperimentEstimatedUniqueVisitorEnteredBucketRecordName($idExperiment));
                }
            }
            return;
        }

        $experiments = Request::processRequest('AbTesting.getAllExperiments', [
            'idSite' => $idSite, 'filter_limit' => -1
        ]);
        foreach ($experiments as $experiment) {
            Request::processRequest('AbTesting.deleteExperiment', [
                'idSite' => $idSite,
    ...
    public function clearAbTestingForSite(bool $isActive, ?int $idSite, string $CompliancePolicy)
    {
        if ($CompliancePolicy !== CnilPolicy::class) {
            return;
        }
        ...
        if (is_null($idSite)) {
            self::endAbTestingForAllSites(true);
        } else {
            self::endAbTestingForSite($idSite, true);
        }
    ...
    public static function endAbTestingForAllSites(bool $skipSettingCheckForActivePolicy = false)
    {
        $sites = SitesManagerAPI::getInstance()->getAllSitesId();
        foreach ($sites as $idSite) {
            self::endAbTestingForSite($idSite, $skipSettingCheckForActivePolicy);
        }
    }
    ...
    public function manipulateJsTracker(&$content)
    {
        ...
        try {
            ...
            if ($featureFlagManager->isFeatureActive('Piwik\Plugins\PrivacyManager\FeatureFlags\PrivacyCompliance')) {
                if (DisableAbTesting::getInstance()->getValue()) {
                    $replaceMethod = 'Matomo.AbTesting.disable();';
                }
            } elseif ($settings->disableAbTesting->getValue()) {
                $replaceMethod = 'Matomo.AbTesting.disable();';
            }
        } catch (\Exception $e) {
            if ($settings->disableAbTesting->getValue()) {
                $replaceMethod = 'Matomo.AbTesting.disable();';
            }
        }

        $content = str_replace([self::TRACKER_READY_HOOK_NAME_WHEN_MINIFIED, self::TRACKER_READY_HOOK_NAME], $replace, $content);
        // since the above replace is at the start of file we cannot use the same replace method as we need to call disable once the tracker is ready
        $content = str_replace([self::TRACKER_READY_HOOK_METHOD_NAME_WHEN_MINIFIED, self::TRACKER_READY_HOOK_METHOD_NAME], $replaceMethod, $content);
    }
    ...
    public function addSubcategories(&$subcategories)
    {
        ...
        $accessValidator = StaticContainer::get(AccessValidator::class);
        if ($accessValidator->isAbTestingDisabled($idSite)) {
            return;
        }
    ...
    public function addWidgetConfigs(&$subcategories)
    {
        ...
        $accessValidator = StaticContainer::get(AccessValidator::class);
        if (!Piwik::isUserHasViewAccess($idSite) || $accessValidator->isAbTestingDisabled($idSite)) {
            return;
        }
    ...
    public function getClientSideTranslationKeys(&$result)
    {
        ...
        $result[] = 'AbTesting_FieldExperimentNameHelpText';
        $result[] = 'AbTesting_FieldNamePlaceholder';
        $result[] = 'AbTesting_FieldHypothesisHelp';
        $result[] = 'AbTesting_FieldHypothesisPlaceholder';
        $result[] = 'AbTesting_FieldDescriptionHelpText';
        ...
        $result[] = 'AbTesting_ForwardAllQueryParams';
        $result[] = 'AbTesting_ForwardAllQueryParamsHelpText';
    }
    ...
    public function updateMatomoTracker()
    {
