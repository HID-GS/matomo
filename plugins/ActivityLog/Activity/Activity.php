...
class Activity
{
     ...
     *
     * Makes it possible to manipulate the stored parameters before they are returned from API
     *
     * @param array $activityData
     * @return array
     */
    ...
    {
        return Common::safe_unserialize($activityData['parameters']);
    }
    ...
    public static function logEvent()
    {
        ...
        // When password is reset, log the reset user_login not anonymous
        if (!empty($activityParameters['password_changed']) && !empty($activityParameters['items'][0]['data']['login'])) {
            $userLogin = $activityParameters['items'][0]['data']['login'];
        }
