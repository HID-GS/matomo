...
class FormsModel
{
    ...
    public function createForm($idSite, $name, $description, $matchFormRules, $matchPageRules, $conversionRuleOption, $conversionRules, $createdDate, $autoCreated, $idGoal)
    {
        $this->validateFormValues($name, $description, $matchFormRules, $matchPageRules, $conversionRuleOption, $conversionRules, $idSite, $idGoal);

        $autoCreated = (bool) $autoCreated;
        $status = self::STATUS_RUNNING;

        $idForm = $this->siteFormDao->createForm($idSite, $name, $description, $status, $matchFormRules, $matchPageRules, $conversionRuleOption, $conversionRules, $createdDate, $autoCreated, $idGoal);
    ...
    public function updateForm($idSite, $idForm, $name, $description, $matchFormRules, $matchPageRules, $conversionRuleOption, $conversionRules, $updatedDate, $idGoal)
    {
        $this->validateFormValues($name, $description, $matchFormRules, $matchPageRules, $conversionRuleOption, $conversionRules, $idSite, $idGoal);
        ...
        $columns = array(
            ...
            'conversion_rule_option' => $conversionRuleOption,
    ...
    private function validateFormValues($name, $description, $matchFormRules, $matchPageRules, $conversionRuleOption, $conversionRules, $idSite, $idGoal)
    {
        $name = new Name($name);
        $name->check();
        ...
        $conversionRuleList = RuleMatcher::getAvailableConversionRuleList();
        if (empty($conversionRuleOption) || !isset($conversionRuleList[$conversionRuleOption])) {
            throw new Exception(Piwik::translate('FormAnalytics_ErrorInvalidConversionOption'));
        }
        ...
        if (is_numeric($idGoal) && empty($this->getGoal($idSite, $idGoal))) {
            throw new Exception(Piwik::translate('FormAnalytics_ErrorGoalInvalid'));
        }
    }
    ...
    {
        $goalInfo = Request::processRequest('Goals.getGoal', array(
            'idSite' => $idSite,
            'idGoal' => $idGoal,
        ));

        return is_array($goalInfo) ? $goalInfo : [];
    }
}
