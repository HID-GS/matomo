...
class Updates_5_1_0 extends PiwikUpdates
{
    ...
    public function getMigrations(Updater $updater)
    {
        $tableName = 'site_form';
        $newColumn = 'idgoal';
        $columns = DbHelper::getTableColumns(Common::prefixTable($tableName));

        $migrations = array();
        if (!array_key_exists($newColumn, $columns)) {
            $migrations[] = $this->migration->db->addColumn($tableName, $newColumn, 'INT NULL', 'idsite');
        }
