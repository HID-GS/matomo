...
class RequestProcessor extends Tracker\RequestProcessor
{
    ...
    public function __construct(LogForm $logForm, LogFormField $logFormField, LogFormPage $logFormPage, FormManager $formManager, Configuration $configuration)
    {
    ...
    private function preprocessFormRequest(VisitProperties $visitProperties, Request $request)
    {
        ...
        if ($this->isFormRequest($request)) {
            ...
            if ($action) {
                ...
                $matchingForms = $this->findMatchingForms($idSite, $params, $pageUrl);
                if (!empty($matchingForms)) {
                    $requestParameterObject = new  \Piwik\Request($params);
                    $isConverted = $requestParameterObject->getIntegerParameter(static::PARAM_FORM_IS_CONVERSION, 0);
                    $isSubmitted = $requestParameterObject->getIntegerParameter(static::PARAM_FORM_IS_SUBMIT, 0);
                    foreach ($matchingForms as $matchingForm) {
                        if (isset($matchingForm['idgoal']) && ($isConverted || $this->isTrackFormSubmitAsFormConversion($matchingForm, $isSubmitted) || $this->manager->isConvertedForm($matchingForm, $pageUrl))) {
                            $goalIds[] = ['idgoal' => $matchingForm['idgoal']];
                        }
                    }
                }
            }
            if (!empty($goalIds)) {
                ...
                $request->setMetadata('Goals', 'goalsConverted', $goalIds);
            }
        }
    ...
    protected function trackForm($form, $idVisitor, $idVisit, $idActionUrl, $serverTime, $params)
    {
        ...
        $isConverted = $this->isTrackFormSubmitAsFormConversion($form, $isSubmitted);

        if (!empty($idLogForm)) {
            $this->logForm->updateRecord($idLogForm, $idFormView, $isViewed, $isStarted, $isSubmitted, $serverTime, $timeHesitation, $timeSpent, $timeToSubmission, $isConverted);
        } else {
            $idLogForm = $this->logForm->addRecord($idVisitor, $idVisit, $idSite, $idSiteForm, $idFormView, $isViewed, $isStarted, $isSubmitted, $serverTime, $timeHesitation, $timeSpent, $timeToSubmission, $isConverted);
        }
    ...
    private function isTrackFormSubmitAsFormConversion($form, $isSubmitted)
    {
        if (
            $isSubmitted &&
            !empty($form['conversion_rule_option']) &&
            $form['conversion_rule_option'] === RuleMatcher::KEY_CONVERSION_RULE_FORM_SUBMIT
        ) {
            return true;
        }
