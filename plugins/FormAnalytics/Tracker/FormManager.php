...
class FormManager
{
    ...
    private function isFormBlockedFromAutoCreation($nameOrId)
    {
        // we are blocking these as they are likely on every page and may not want to be tracked and cause a lot of load
        // they can still be tracked but need to be created manually
        if (empty($nameOrId)) {
            return false;
        }
        $nameOrId = Common::mb_strtolower($nameOrId);
        if ($nameOrId === 'form1') {
            return true; // we assume this might be a search
        }
        $nameOrId = preg_replace("/[^a-z]+/", "", $nameOrId);
        if (empty($nameOrId)) {
            return false;
        }
        $blockedForms = [
            'formsearch', 'searchform', 'search',
            'searchdesktop', 'searchmobile', 'mobilesearch', 'desktopsearch',
            'sitesearch', 'generalsearch', 'searchgeneral', 'searchsite', // all site search,
        ];
        $blockedFormsWithVariations = [];
        foreach ($blockedForms as $blockedForm) {
            $blockedFormsWithVariations[] = $blockedForm;
            $blockedFormsWithVariations[] = $blockedForm . 'id';
            $blockedFormsWithVariations[] = $blockedForm . 'name';
            $blockedFormsWithVariations[] = 'name' . $blockedForm;
            $blockedFormsWithVariations[] = 'id' . $blockedForm;
        }
        if (in_array($nameOrId, $blockedFormsWithVariations, true)) {
            return true;
        }

        $blockedContains = [
            'suchen', 'chercher', 'ricerca', 'buscar', // search in few different languages
            'anmelden', 'login' // login form can sometimes also be on every page. eg loginform or formlogin
        ];
        foreach ($blockedContains as $blockedContain) {
            if (strpos($nameOrId, $blockedContain) !== false) {
                return true;
            }
        }
    ...
    public function autoCreateForm($idSite, $formName, $formId, $pageUrl)
    {
        if (!empty($formName) || !empty($formId)) {
            ...
            if (!empty($formId) && strpos($formId, 'hsForm_') === 0) {
            ...
            if ($setting === SystemSettings::FORM_CREATION_UP_TO_3 && $numAutoCreated >= 3) {
                return;
            }
            ...
            if ($setting === SystemSettings::FORM_CREATION_UP_TO_150 && $numAutoCreated >= 150) {
                return;
            }

            $shouldCreateForm = true;
            Piwik::postEvent('FormAnalytics.shouldAutoCreateForm', array(&$shouldCreateForm, array('idSite' => $idSite, 'formName' => $formName, 'formId' => $formId, 'pageUrl' => $pageUrl)));
            ...
            if (!$this->doesAnyFormMatchFormRules($idSite, $formName, $formId)) {
                ...
                try {
                    $idForm = $this->model->createForm($idSite, $name, $description, $matchFormRules, $matchPageRules = [], RuleMatcher::KEY_CONVERSION_RULE_PAGE_VISIT, $conversionRules = [], $createdDate, $autoCreated = true, '');
