...
class SiteForm
{
    ...
    public function install()
    {
        DbHelper::createTable($this->table, "
                  ...
                  `conversion_rule_option` VARCHAR(50) NOT NULL DEFAULT '" . RuleMatcher::KEY_CONVERSION_RULE_PAGE_VISIT . "',
     ...
     * @return array
     */
    public function getFormIdsWithStatus($idSite, $status)
    {
        $bind = array($status, $idSite);

        $table = $this->tablePrefixed;
        $forms = $this->getDb()->fetchAll("SELECT idsiteform FROM $table WHERE status = ? AND idsite = ?", $bind);

        $ids = array_column($forms, 'idsiteform');
    ...
    public function createForm($idSite, $name, $description, $status, $matchFormRules, $matchPageRules, $conversionRuleOption, $conversionRules, $createdDate, $autoCreated, $idGoal)
    {
        $columns = array(
            'idsite' => $idSite,
            'name' => $name,
    ...
    public function removeGoalFromMatchingForms($idSite, $idGoal)
    {
        $query = sprintf('UPDATE %s SET idgoal=NULL WHERE idgoal = ? AND idsite = ?', $this->tablePrefixed);
        $bind = [$idGoal, $idSite];
    ...
    private function encodeFieldsWhereNeeded($columns)
    {
        foreach ($columns as $column => $value) {
            if (in_array($column, array('match_form_rules', 'match_page_rules', 'conversion_rules', 'fields'))) {
                if ($column === 'fields') {
                    if (!empty($value) && is_array($value) && count($value) > 2500) {
                        // restrict to max 2500 form fields, otherwise the update may fail
                        $value = array_slice($value, 0, 2500);
                    }
                }
