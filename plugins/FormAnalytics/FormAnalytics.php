...
class FormAnalytics extends Plugin
{
    ...
    public function registerEvents()
    {
        return array(
            ...
            'API.Goals.deleteGoal.end' => 'removeGoalFromMatchingForms'
        );
    }
    ...
    {
        // we cache running forms separately for matching during tracking, but include deleted and archived forms in
        // the all-forms cache so deleted/archived auto-created forms are not imported again.
        $formsModel = $this->getFormsModel();
        $content[self::TRACKER_CACHE_RUNNING_FORMS_KEY] = $formsModel->getFormsByStatuses($idSite, FormsModel::STATUS_RUNNING);
        $content[self::TRACKER_CACHE_ALL_FORMS_KEY] = $formsModel->getFormsByStatuses(
            $idSite,
            array(FormsModel::STATUS_RUNNING, FormsModel::STATUS_ARCHIVED, FormsModel::STATUS_DELETED)
        );
    ...
    public function getClientSideTranslationKeys(&$result)
    {
        ...
        $result[] = 'FormAnalytics_FieldDescriptionPlaceholderV2';
        $result[] = 'FormAnalytics_FieldNamePlaceholderV2';
        ...
        $result[] = 'FormAnalytics_FormNameHelpText';
        $result[] = 'FormAnalytics_FormDescriptionHelpText';
        ...
        $result[] = 'FormAnalytics_SegmentedVisitorLogPopupTitleFormStarter';
        $result[] = 'FormAnalytics_SegmentedVisitorLogPopupTitleFormSubmitters';
        $result[] = 'FormAnalytics_SegmentedVisitorLogPopupTitleFormConverters';
        $result[] = 'FormAnalytics_GoalConversionRulesTitle';
        $result[] = 'FormAnalytics_GoalConversionRulesDescription';
    }
    ...
    public function getJsFiles(&$jsFiles)
    {
        ...
        $jsFiles[] = "plugins/FormAnalytics/javascripts/faSegmentedVisitorLog.js";
    }
    ...
    public function removeGoalFromMatchingForms($returnedValue, $info)
    {
        ...
        if (empty($goal['idgoal'])) {
            // we only update forms if that goal was actually deleted
            // we check for idgoal because API might return true even though goal does not exist
            $siteFormDao = $this->getSiteFormsDao();
            $siteFormDao->removeGoalFromMatchingForms($idSite, $idGoal);
        }
    }
}
