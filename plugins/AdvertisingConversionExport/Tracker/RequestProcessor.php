...
class RequestProcessor extends Tracker\RequestProcessor
{
    ...
    public function manipulateRequest(Request $request)
    {
        ...
        if (false === AdvertisingConversionExport::isVisitUpdateAndExportDisabled($idSite)) {
     ...
     * @return null|array
     */
    protected function getClickIdAndProviderFromRequest(Request $request): ?array
    {
        ...
        if (empty($rawParams['url']) || !is_string($rawParams['url'])) {
            return null; //as there is no point checking further if url is empty
        }
