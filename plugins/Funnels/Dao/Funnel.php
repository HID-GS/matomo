...
class Funnel
{
    ...
    {
        $table = $this->tablePrefixed;
        $funnel = $this->getDb()->fetchRow("SELECT * FROM $table WHERE idgoal = 0 AND idsite = ? AND deleted = 0 AND name IS NULL", [$idSite]);
    ...
    public function createNonGoalFunnel(int $idSite, string $funnelName, bool $isActivated, array $steps, string $createdDate, ?string $description = null)
    {
        $db = $this->getDb();
        // The key difference between this funnel and a Sales funnel is that this funnel has a name value set
    ...
    private function setSteps($idFunnel, $steps)
    {
        ...
        if (!empty($steps)) {
            ...
            foreach ($steps as $index => $step) {
                ...
                $position++;
                $this->steps->insertStep($idFunnel, $position, trim($step['name']), trim($step['pattern']), $step['pattern_type'], $step['required']);
            }
        }
    }

    private function incrementRevisionNumber(int $idFunnel)
    {
        $table = $this->tablePrefixed;
        $query = "UPDATE $table SET `revision` = `revision` + 1 WHERE idfunnel = ?";

        $this->getDb()->query($query, [$idFunnel]);
    }

    public function updateFunnel($idFunnel, $isActivated, $steps, $shouldIncrementRevision = false)
    {
        ...
        if ($shouldIncrementRevision) {
            $this->incrementRevisionNumber($idFunnel);
        }
    }

    public function updateFunnelName(int $idFunnel, string $name)
    {
        $this->getDb()->update($this->tablePrefixed, [
            'name' => $name
        ], "idfunnel = $idFunnel");
    }

    public function updateFunnelDescription(int $idFunnel, ?string $description)
    {
        $this->getDb()->update($this->tablePrefixed, [
            'description' => $description
        ], "idfunnel = $idFunnel");
