...
use Piwik\Plugins\Funnels\Input\Description;
use Piwik\Site;
...
class FunnelsModel
{
    ...
    public function getStoredGoalFunnel($idSite, $idGoal)
    {
        return $this->funnelDao->getGoalFunnel($idSite, $idGoal);
    }
    ...
    public function saveNonGoalFunnel($idSite, $idFunnel, $isActivated, $steps, $now, $funnelName, $shouldIncrementRevision, $description = null)
    {
        $descriptionInput = new Description($description);
        $descriptionInput->check();
        ...
        if (!empty($idFunnel)) {
            ...
            $this->funnelDao->updateFunnelDescription($idFunnel, $description);
            $this->getFunnelsTransientCache()->delete($this->getCacheId($idSite));
            return $idFunnel;
        }

        $idFunnel = $this->funnelDao->createNonGoalFunnel($idSite, $funnelName, $isActivated, $steps, $now, $description);
