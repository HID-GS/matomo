...
class GetLastReappearedCrashes extends Base
{
    protected function init()
    {
        parent::init();

        $this->name = Piwik::translate('CrashAnalytics_LastReappearedCrashes');
        $this->dimension = new Crash();
        $this->documentation = Piwik::translate('CrashAnalytics_LastReappearedCrashesDocumentation');
    ...
    public function configureView(ViewDataTable $view)
    {
        ...
        $view->config->columns_to_display = ['label', 'crash_source', 'crash_last_reappeared', Metrics::CRASH_OCCURRENCES];

        $view->requestConfig->filter_sort_column = 'crash_last_reappeared';
    ...
    public function configureWidgets(WidgetsList $widgetsList, ReportWidgetFactory $factory)
    {
        ...
        foreach ($lastNMinutes as $index => $timeToAdd) {
            ...
            if ($timeToAdd < 60) {
                $config->setName(Piwik::translate('CrashAnalytics_ReappearedCrashesLastNMinutes', $timeToAdd));
            } else {
                $config->setName(Piwik::translate('CrashAnalytics_ReappearedCrashesLastNHours', floor($timeToAdd / 60)));
            }
