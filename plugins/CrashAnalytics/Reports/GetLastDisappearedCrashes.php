...
class GetLastDisappearedCrashes extends Base
{
    protected function init()
    {
        parent::init();

        $this->name = Piwik::translate('CrashAnalytics_LastDisappearedCrashes');
        $this->dimension = new Crash();
        $this->documentation = Piwik::translate('CrashAnalytics_LastDisappearedCrashesDocumentation');
    ...
    public function configureView(ViewDataTable $view)
    {
        ...
        $view->config->columns_to_display = ['label', 'crash_source', 'crash_last_seen'];

        $view->requestConfig->filter_sort_column = 'crash_last_seen';
    ...
    public function configureWidgets(WidgetsList $widgetsList, ReportWidgetFactory $factory)
    {
        ...
        foreach ($lastNMinutes as $index => $timeToAdd) {
            ...
            if ($timeToAdd < 60) {
                $config->setName(Piwik::translate('CrashAnalytics_DisappearedCrashesLastNMinutes', $timeToAdd));
            } else {
                $config->setName(Piwik::translate('CrashAnalytics_DisappearedCrashesLastNHours', floor($timeToAdd / 60)));
            }
