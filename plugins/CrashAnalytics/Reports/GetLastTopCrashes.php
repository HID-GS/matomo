...
class GetLastTopCrashes extends Base
{
    protected function init()
    {
        parent::init();

        $this->name = Piwik::translate('CrashAnalytics_LastTopCrashes');
        $this->dimension = new Crash();
        $this->documentation = Piwik::translate('CrashAnalytics_LastTopCrashesDocumentation');
    ...
    public function configureView(ViewDataTable $view)
    {
        ...
        $view->config->columns_to_display = ['label', 'crash_source', Metrics::CRASH_OCCURRENCES, Metrics::VISITS_WITH_CRASH];

        $view->requestConfig->filter_sort_column = Metrics::CRASH_OCCURRENCES;
    ...
    public function configureWidgets(WidgetsList $widgetsList, ReportWidgetFactory $factory)
    {
        ...
        foreach ($lastNMinutes as $index => $timeToAdd) {
            ...
            if ($timeToAdd < 60) {
                $config->setName(Piwik::translate('CrashAnalytics_TopCrashesLastNMinutes', $timeToAdd));
            } else {
                $config->setName(Piwik::translate('CrashAnalytics_TopCrashesLastNHours', floor($timeToAdd / 60)));
            }
