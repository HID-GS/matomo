...
use Piwik\Plugins\CrashAnalytics\Columns\CrashCategory;
use Piwik\Plugins\CrashAnalytics\Columns\Metrics\VisitsCrashRate;
use Piwik\Plugins\CrashAnalytics\Metrics;
use Piwik\Report\ReportWidgetFactory;
use Piwik\Widget\WidgetsList;

class GetCrashesByCategory extends Base
{
    protected function init()
    {
        parent::init();

        $this->dimension = new CrashCategory();
        $this->metrics = [Metrics::CRASH_OCCURRENCES, Metrics::VISITS_WITH_CRASH];
        $this->name = Piwik::translate('CrashAnalytics_CrashesByCategory');
        $this->documentation = Piwik::translate('CrashAnalytics_CrashesByCategoryDocumentation');
        $this->subcategoryId = 'CrashAnalytics_AllCrashes';
        $this->actionToLoadSubTables = 'getCrashesForCategory';
        $this->order = 20;
    ...
    public function configureWidgets(WidgetsList $widgetsList, ReportWidgetFactory $factory)
    {
        ...
        $widget->setName('CrashAnalytics_Category');
