...
class LogCrash
{
    public const TABLE_NAME = 'log_crash';

    public const DEFAULT_DAYS_UNTIL_CONSIDERED_DISAPPEARED = 7;

    public const MAX_LENGTH_MESSAGE = 255;
    public const MAX_LENGTH_RESOURCE_URI = 300;
    public const MAX_LENGTH_CRASH_TYPE = 100;
    public const DEFAULT_UPDATE_EVERY_N_HOURS = 3;
    ...
    public function __construct(
        BrowserInconsistencies $browserInconsistencies,
        LogCrashGroup $logCrashGroup,
    ...
    private function findEntry($crc32hash, $normalizedMessage, $resourceUri, $idSite)
    {
        ...
        foreach ($rows as $row) { // handle hash collisions
            ...
            if (
                $normalizedMessage == $rowNormalizedMessage
    ...
    public function searchCrashMessagesForMerge(
        int $idSite,
        string $searchTerm,
        string $resourceUri,
        int $filter_limit,
        int $filter_offset,
        array $excludeIdLogCrashes = []
    ): array {
        $extraWhere = '';
    ...
    public function getCountofCrashesByIdSite($idSite, array $idLogCrashes): int
    {
        $idLogCrashes = array_map('intval', $idLogCrashes);
        ...
        $sql = 'SELECT COUNT(*) FROM ' . Common::prefixTable(self::TABLE_NAME)
            . ' WHERE idsite = ? AND idlogcrash IN (' . implode(',', $idLogCrashes) . ')';

        return (int) $this->getDb()->fetchOne($sql, [$idSite]);
    }
