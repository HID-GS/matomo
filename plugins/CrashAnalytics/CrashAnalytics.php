...
{
    public const LIMIT_IGNORED_HASHES_IN_TRACKER = 200;
    public const TRACKER_READY_HOOK_NAME = '/*!! crashAnalyticsTrackerReadyHook */';
    public const TRACKER_READY_HOOK_NAME_WHEN_MINIFIED = '/*!!! crashAnalyticsTrackerReadyHook */';
    public const RATE_OF_PAGEVIEW_CRASH_FOR_ENTERPRISE_DEMO = 3; // 3%

    /**
     * @var \Faker\Generator | null
     */
    private static $enterpriseDemoFaker = null;
    ...
    public function addJsGlobalVariables(&$output)
    {
        ...
        $globalVars = [
            ...
            'isCrashAnalyticsDisabled' => false,
        ];
        if (StaticContainer::get(SystemSettings::class)->disableCrashAnalytics->getValue()) {
            $globalVars['isCrashAnalyticsDisabled'] = true;
            $globalVars['crashAnalyticsDisabledNotificationMessage'] = Piwik::translate('CrashAnalytics_ErrorCrashAnalyticsDisabledNonSuperUser');
            if (Piwik::hasUserSuperUserAccess()) {
                ...
                $globalVars['crashAnalyticsDisabledNotificationMessage'] = Piwik::translate('CrashAnalytics_ErrorCrashAnalyticsDisabledSuperUser', ['<a href="' . $settingsUrl . '" target="_blank" rel="noopener noreferrer">', '</a>']);
            }
        }
    ...
    public function modifyTrackerJs(&$content)
    {
        ...
        if ($settings->disableCrashAnalytics->getValue()) {
            $replace[] = 'Matomo.CrashAnalytics.disable();';
        }
        Piwik::postEvent('CrashAnalytics.modifyTrackerJs', [&$replace]);
        if (empty($replace)) {
            return;
        }

        $content = str_replace(array(self::TRACKER_READY_HOOK_NAME_WHEN_MINIFIED, self::TRACKER_READY_HOOK_NAME), implode("\n", $replace), $content);
    }
    ...
    public function getJsFiles(&$jsFiles)
    {
        ...
        $jsFiles[] = "plugins/CrashAnalytics/javascripts/CrashAnalyticsDisabledNotification.js";
    }
    ...
    {
        if (empty(self::$enterpriseDemoFaker)) { // If null do lazy init
