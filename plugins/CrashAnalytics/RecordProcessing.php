...
class RecordProcessing
{
     ...
     * @return void
     */
    public function filterCrashes($dataTable, string $pattern): void
    {
        // Ignore standard Pattern filter requests, we'll do our own column filtering below
        $dataTable->disableFilter('Pattern');
        $searchColumns = ['label', 'crash_type', 'crash_source'];
        if ($pattern) {
            $dataTable->filter('ColumnCallbackDeleteRow', array($searchColumns, function (...$columnValues) use ($pattern) {
                foreach ($columnValues as $value) {
                    if (str_contains((string) mb_strtolower($value), mb_strtolower($pattern))) {
                        return false;
                    }

                    // Tried normal search for unknown, now search for empty values
                    if (mb_strtolower($pattern) === mb_strtolower(Piwik::translate('General_Unknown')) && (string) $value === '') {
                        return false;
                    }
                }
                return true;
