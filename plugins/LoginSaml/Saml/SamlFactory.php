...
use Piwik\Date;
use Piwik\Piwik;
use Piwik\Plugins\LoginSaml\PassthroughAuth;
...
class SamlFactory
{
    ...
    public function getSamlSettings()
    {
        ...
        // Listen HTTP_X_FORWARDED_PORT and HTTP_X_FORWARDED_PROTO
        // if advanced_set_proxy_vars enabled
        if ($configData['advanced_set_proxy_vars']) {
        ...
        $metadata = array(
            ...
            'sp' => array(
                ...
                */
                ...
                'x509cert' => Utils::formatCert($configData['advanced_sp_x509cert']),
                'privateKey' => Utils::formatPrivateKey($configData['advanced_sp_privatekey']),
            ),
            'idp' => array(
        ...
        $idpX509cert = $configData['idp_x509cert'];
        $idpX509cert2 = $configData['idp_x509cert2'];
        $idpX509cert3 = $configData['idp_x509cert3'];

        $metadata['idp']['x509certMulti'] = [
              'signing' => [
                  0 => Utils::formatCert($idpX509cert),
              ],
              'encryption' => [
                  0 => Utils::formatCert($idpX509cert),
              ]
        ...
        if (!empty($idpX509cert2)) {
            $metadata['idp']['x509certMulti']['signing'][] = Utils::formatCert($idpX509cert2);
        }
        if (!empty($idpX509cert3)) {
            $metadata['idp']['x509certMulti']['signing'][] = Utils::formatCert($idpX509cert3);
        }
    ...
    public function synchronizePiwikAccessFromSaml($user, $attributes)
    {
        ...
        // Reset current access first so the IdP remains authoritative even when it sends no access attributes.
    ...
    public function assignDefaultSitesViewAccessIfApplies($user)
    {
        ...
        if (!empty($defaultSitesWithViewAccess)) {
            ...
            $allIds = Access::doAsSuperUser(function () {
    ...
    {
        $passthroughAuth = new PassthroughAuth($user);
    ...
    /**
     * Gets the {@link $configData} property.
     *
     * @return array
     */
    public function geConfigData()
    {
        return $this->configData;
    }

    private function generateRandomHashedPassword()
    {
        $randomPassword = md5(
