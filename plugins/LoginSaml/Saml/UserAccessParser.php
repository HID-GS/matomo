 ...
 *     write: piwikD.myhost.com/path/to/piwik:1,2
...
class UserAccessParser
{
    ...
    public function __construct($configData, $logger, $samlAttributes)
    {
        ...
        if (empty($configData['saml_write_access_field'])) {
            $configData['saml_write_access_field'] = 'write';
        }
    ...
    public function getPiwikUserAccessForSamlUser()
    {
        ...
        $writeAccessValue = $this->retrieveWriteAccessValue();
        if (!empty($writeAccessValue)) {
            $this->addSiteAccess($sitesByAccess, 'write', $writeAccessValue);
        }
    ...
    public function retrieveWriteAccessValue()
    {
        return $this->retrieveAccessValue('saml_write_access_field');
    }
