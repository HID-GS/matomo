...
class Config
{
    public static $defaultConfig = array(
        ...
        'advanced_set_proxy_vars' => 0,
        'enable_password_confirmation' => 1,
        'advanced_use_friendlyname' => 0,
        'advanced_login_saml_button_text' => '',
        'options_bypass_2fa' => 0
    );
    ...
    {
        if ($optionName === 'advanced_login_saml_button_text' && empty($config[$optionName])) {
            return Piwik::translate('LoginSaml_LoginWithSSO');
        } elseif (isset($config[$optionName])) {
    ...
    {
        // Only for UI test, default value should be always 0
        if ($optionName === 'enable_password_confirmation_force') {
            return 0;
        }
    ...
    {
        $configData = self::getPluginOptionValuesWithDefaults();
        return (isset($configData['status']) && $configData['status'] && isset($configData['options_forcesaml']) && $configData['options_forcesaml']);
    }

    public static function isPreventNonSuperUsersEnabled()
    {
        $configData = self::getPluginOptionValuesWithDefaults();
        return (isset($configData['status']) && $configData['status'] && isset($configData['options_preventnonsuperusers']) && $configData['options_preventnonsuperusers']);
    }

    public static function isPreventSuperUsersEnabled()
    {
        $configData = self::getPluginOptionValuesWithDefaults();
        return (isset($configData['status']) && $configData['status'] && isset($configData['options_preventsuperusers']) && $configData['options_preventsuperusers']);
    }
    ...
    {
        $configData = self::getPluginOptionValuesWithDefaults();
        return (isset($configData['options_bypass_2fa']) && $configData['options_bypass_2fa']);
    }
    ...
    public static function savePluginOptions($config)
    {
        ...
        // Format certs properly
        $loginSaml = self::formatCerts($loginSaml);
    ...
    public static function injectSamlValues($values, $resetIdPValues = false)
    {
        ...
        if (isset($values['idp'])) {
            if ($resetIdPValues) {
                ...
                $loginSaml['idp_x509cert2'] = '';
                $loginSaml['idp_x509cert3'] = '';
            }
            ...
            if (isset($values['idp']['x509certMulti'])) {
                if (isset($values['idp']['x509certMulti']['signing'])) {
                    if (isset($values['idp']['x509certMulti']['signing'][0]) && !empty($values['idp']['x509certMulti']['signing'][0])) {
                        $loginSaml['idp_x509cert'] = $values['idp']['x509certMulti']['signing'][0];
                    }
                    if (isset($values['idp']['x509certMulti']['signing'][1]) && !empty($values['idp']['x509certMulti']['signing'][1])) {
                        $loginSaml['idp_x509cert2'] = $values['idp']['x509certMulti']['signing'][1];
                    }
                    if (isset($values['idp']['x509certMulti']['signing'][2]) && !empty($values['idp']['x509certMulti']['signing'][2])) {
                        $loginSaml['idp_x509cert3'] = $values['idp']['x509certMulti']['signing'][2];
                    }
                }
                if (isset($values['idp']['x509certMulti']['encryption']) && !empty($values['idp']['x509certMulti']['encryption'][0])) {
                    $loginSaml['idp_x509cert'] = $values['idp']['x509certMulti']['encryption'][0];
                }
            }

            if (isset($values['idp']['x509cert2']) && !empty($values['idp']['x509cert2'])) {
                $loginSaml['idp_x509cert2'] = $values['idp']['x509cert2'];
            }
            if (isset($values['idp']['x509cert3']) && !empty($values['idp']['x509cert3'])) {
                $loginSaml['idp_x509cert3'] = $values['idp']['x509cert3'];
            }
        }
    ...
    private static function formatCerts($loginSaml)
    {
        ...
        if (!empty($loginSaml['idp_x509cert2'])) {
            $loginSaml['idp_x509cert2'] = Utils::formatCert($loginSaml['idp_x509cert2'], false);
        }
        if (!empty($loginSaml['idp_x509cert3'])) {
            $loginSaml['idp_x509cert3'] = Utils::formatCert($loginSaml['idp_x509cert3'], false);
        }
        if (!empty($loginSaml['advanced_sp_x509cert'])) {
            $loginSaml['advanced_sp_x509cert'] = Utils::formatCert($loginSaml['advanced_sp_x509cert'], false);
        }
        if (!empty($loginSaml['advanced_sp_privatekey'])) {
            $loginSaml['advanced_sp_privatekey'] = str_replace(["\x0D", "\r", "\n"], "", $loginSaml['advanced_sp_privatekey']);
        }
