...
## Using local IdP for testing during development
* Start the container using something like `docker compose -p matomo-saml -f .dev-environment/docker-compose.test-saml-idp.yaml up -d`
* Access the Keycloak admin console at http://localhost:8082/ and log in using admin: admin as credentials
* Make sure that you have a SAML IdP by going to Identity providers > User-defined > SAML v2.0
* Import the http://localhost:8082/realms/dev/protocol/saml/descriptor metadata into Matomo at Administration > SAML > Identity Provider Settings `/index.php?module=LoginSaml&action=importmetadata`
* Under Attribute Mapping Settings, set Login (username) = `username` and Email = `email` and adjust any other desired settings
* Download the SP metadata XML from Matomo under SAML Status > Access to SP metadata `/index.php?module=LoginSaml&action=metadata`
* In Keycload, import the metadata XML at Manage > Clients and clicking the Import client button
* Make sure that the `Test-Matomo-SAML` client scope is the default scope of the newly created client
* Make any other desired changes and ensure that Matomo SAML authentication is enabled at Administration > SAML > SAML Status

_NOTE:_ If you're using the Matomo DDEV environment, you can use something like the following in your .ddev/config.local.yaml file to start the container:
```
hooks:
    post-start:
        # Start the test IdP on the HOST with its own compose project name.
        - exec-host: |
            set -e
            env -u COMPOSE_PROJECT_NAME -u COMPOSE_FILE -u DOCKER_COMPOSE_FILE \
            docker compose \
                -p matomo-saml \
                -f plugins/LoginSaml/.dev-environment/docker-compose.test-saml-idp.yaml \
                up -d
```