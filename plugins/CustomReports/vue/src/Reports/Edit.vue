            ...
            <div v-if="report.allowedToEdit || multipleSites.length">
              ...
              <table class="entityTable">
                ...
                <tr
                  ...
                  :key="index"
                  :class="{ selectedSiteRow: isSelectedSiteRow(site) }"
                >
                  <td>{{ site.idsite }}</td>
                  <td>{{ site.name }}</td>
                  <td class="siteAction" v-if="report.allowedToEdit">
                    <span
                      v-if="canRemoveSite(site)"
        ...
        <div class="form-group row" v-if="childReports.length">
          ...
          <div class="col s12 m6">
            <DraggableList
              ...
              @reorder="onChildReportsReorder"
            >
              <template #default="{ item: childReport }">
                <span class="icon-menu-hamburger drag-icon"></span>
                <span>{{ childReport.name }}</span>
              </template>
            </DraggableList>
          </div>
          <div class="col s12 m6">
...
export default defineComponent({
  ...
  methods: {
    ...
    syncChildReportIds(reports?: ChildReport[]) {
      const sourceReports = reports ?? this.childReports;
      this.childReportIds = sourceReports
        .map((childReport: ChildReport) => childReport.idcustomreport);
    },
    doUnlock() {
      this.isLocked = false;
    ...
    init() {
      ...
      if (this.create) {
        ...
        this.multipleSites = [{
          idsite: this.report.site.id as string|number,
          name: this.report.site.name as string,
        }];
        this.isLocked = false;
        this.canEdit = true;
        this.isDirty = false;
    ...
    onChildReportsReorder(order: string[]) {
      const reportsById = new Map(
        this.childReports.map((childReport) => [`${childReport.idcustomreport}`, childReport]),
      );
      const reorderedReports = order
        .map((id) => reportsById.get(id))
        .filter((childReport): childReport is ChildReport => !!childReport);
    ...
    updateReport() {
      ...
      CustomReportsStore.createOrUpdateReport(
        ...
        const idSite = this.report.site.id;
        const currentSiteRemainsAccessible = this.multipleIdSites.some(
          (siteId) => `${siteId}` === `${Matomo.idSite}`,
        );
        this.isDirty = false;
        this.canEdit = true;
    ...
    isSelectedSiteRow(site: Site) {
      return !!this.multipleSites
        && this.multipleSites.length > 1
        && `${site.idsite}` === `${this.report.site.id}`;
    },
    canRemoveSite(site: Site) {
    ...
    removeSite(site: Site) {
      if (this.multipleSites) {
        ...
        if (isSelectedSite && this.multipleSites.length) {
          const previousSite = removedIndex > 0
            ? this.multipleSites[removedIndex - 1]
            : null;
