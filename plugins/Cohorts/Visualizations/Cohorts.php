...
class Cohorts extends HtmlTable
{
    ...
    public function getCellHtmlAttributes(Row $row, $column)
    {
        ...
        $isLowerBetter = $row->getMetadata('isLowerBetter');
        $total = $row->getMetadata('maxValue');

        $formattedColumnValue = $row->getColumn($column) ?: 0;
        $columnValue = \Piwik\Plugins\Cohorts\Cohorts::getNumericValue($formattedColumnValue);

        $columnValueRatio = $total == 0 ? 0 : min($columnValue / $total, 1.0);
        // If lower is better for this metric, reverse the ratio
        $columnValueRatio = !$isLowerBetter || $row->getColumn($column) === false ? $columnValueRatio : 1 - $columnValueRatio;
        ...
        // This is a hack to get the HtmlTable to display 0 instead of hyphen
        if ($row->hasColumn($column) && empty($row->getColumn($column))) {
