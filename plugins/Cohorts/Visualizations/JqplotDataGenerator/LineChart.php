...
class LineChart extends JqplotDataGenerator\Evolution
{
    ...
    protected function initChartObjectData($dataTable, $visualization)
    {
        ...
        // If there are no cohorts with data, add one empty point so that the chart actually displays
        if (empty($allSeriesData)) {
            // Get the series label using the requested metric. We might not have the cohort name if no rows were returned
            $columnName = $this->properties['request_parameters_to_modify']['metrics'] ?? $columnsToDisplay[0];
            $seriesLabel = $this->getSeriesLabel($rowsToDisplay[0], $columnName);
    ...
    protected function getSeriesData($rowLabel, $columnName, DataTable\Map $dataTable)
    {
        ...
        // Work backwards through the series removing each entry until non-zero data is found
        for ($i = count($seriesData) - 1; $i >= 0; $i--) {
            $value = $seriesData[$i] ?? '';
            if (!empty($value)) {
                break;
            }
