...
class Controller extends \Piwik\Plugin\Controller
{
    public function index()
    {
        ...
        foreach ($limits as $limit) {
            $limitValues[] = $limit . ' ' . Piwik::translate('Cohorts_Cohorts');
        }
        ...
        return $this->renderTemplate('index', [
            ...
            'limits' => array_combine($limits, $limitValues),
    ...
    public function getCohortsChart()
    {
        ...
        // Get the defaults in case the query parameters aren't present
        $savedParams = GetCohortsTable::getSavedDefaultReportParameters();
        // Don't use the same defaults if it's a widget
        $isWidget = $request->getBoolParameter('widget', false);
        if ($isWidget) {
            $savedParams = [
                'metric' => GetCohorts::DEFAULT_METRIC,
                'filter_limit' => 10,
            ];
        }
