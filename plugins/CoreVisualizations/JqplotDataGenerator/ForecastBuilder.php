 ...
 * - Non-monotonic series (ratios, averages, latency-style metrics — anything tagged percent or
 *   "lower value is better" by the caller): the forecast is the historical same-period average
 *   only, and is allowed to fall below the current display value because the period's value
 *   can move in either direction during the remaining time.
 *
...
class ForecastBuilder
{
     ...
     *        series is treated as non-monotonic and forecasts are produced from the historical
     *        prior only without the "forecast >= current" suppression. When the flag is missing
     *        for a series, the builder falls back to "percent unit implies non-monotonic".
