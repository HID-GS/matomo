...
class Access
{
    ...
    private function applyTokenAccessLevelRestrictionToLoadedSites(): void
    {
        ...
        if ($this->isSuperUserRestrictedByTokenAccessLevel()) {
            try {
                ...
                $allSiteIds = (new SitesManagerModel())->getSitesId();
            } catch (\Exception $e) {
            ...
            // Sites not present in the explicit access rows have implicit superuser access.
            foreach ($allSiteIds as $idSite) {
                if (!isset($siteRoleMap[$idSite])) {
    ...
    {
        $map = [];
        foreach (array_reverse(self::getTokenAccessLevels()) as $role) {
     ...
     * and separately granted capability rows are ignored.
     *
    ...
    private function resolveTokenAccessLevelFromSubmittedToken(): ?string
    {
        ...
        try {
            $metadata = (new UsersModel())->getTokenMetadataByTokenAuth($submittedToken);
        } catch (\Exception $e) {
            StaticContainer::get(LoggerInterface::class)->debug(
